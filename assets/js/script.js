...
function initFHACalculations() {
    ...
    let fha_monthly_tc_fee_inp = parseFloat($('#fha_monthly_tc_fee_inp').val()) || 0;
    ...
    let fha_seller_agent_comp_offset_inp = parseFloat($('#fha_seller_agent_comp_offset_inp').val()) || 0;
    ...
    // Calculations
    let fha_base_loan_amnt_inp = fha_sales_price_inp - (fha_sales_price_inp * (fha_est_dwn_pmt_inp / 100));
    $('#fha_base_loan_amnt_inp').val(fha_base_loan_amnt_inp > 0 ? fha_base_loan_amnt_inp.toFixed(2) : '');

    let fha_mip_loan_amnt_inp = (fha_est_dwn_pmt_inp / 100) >= 0.2 ? 0 : fha_base_loan_amnt_inp * 1.0175;
    ...
    if (fha_sales_price_inp !== 0) {
        if ((fha_est_dwn_pmt_inp / 100) < 0.2) {
            fha_monthly_mip = fha_principle_interest_inp * 0.085;
        }
    }
    $('#fha_monthly_mip_inp').val(`${fha_monthly_mip.toFixed(2)}`);

    let est_total_pmt = fha_principle_interest_inp + fha_monthly_ins_inp + fha_monthly_taxes_inp + fha_monthly_mip + fha_monthly_tc_fee_inp;
    $('#est_total_pmt').html(`${est_total_pmt.toFixed(2)}`);
    ...
    let fha_down_pmt_inp = fha_sales_price_inp * fha_est_dwn_pmt_inp / 100;
    ...
    let fha_reserves_cash_on_hand = 2 * fha_monthly_tc_fee_inp;
    ...
    // console.log(fha_loan_amount, fha_est_dwn_pmt_inp, fha_monthly_tc_fee_inp, fha_inspection_inp, fha_appraisal_inp, fha_option_money_inp, fha_yearly_ins_inp, fha_yearly_taxes_inp, fha_mortgage_years_inp, fha_mortgage_rate_inp, fha_est_sbac_inp);
}
